...
Another downside is, that currently you need to configure what opertations shall be locked at the compile time of TSan. When changing this configuration TSan itself needs to be recomiled, which can take a while. In order to improve this it would be better to allow for this configuration of which actions shall be logged with parameters when calling the compiled program. Before doing this improvement it should be measured if this is actually reasonable or if this slows the prgram down. A slow down could happen because further code needs to be executed at the actual runtime of TSan. 
