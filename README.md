...
1. This includes the file [log.h](compiler-rt/lib/tsan/rtl/log.h) where you can comment in and out the lines, that define what actions shall be logged. Possible options for events that can be logged are:
   1. Read operations on variables
   2. Write operations on variables
   3. Mutex lock and unlock operations.
   4. Thread Fork operations (creation of a new Thread)
   5. Thread Join operations 
...
4. Because TSan uses some wrapper functionality to handle the Thread fork and join events as well as the Mutex lock and unlock events, the code position that was written in the log message was not correct and instead pointed to the code position of the wrapper. To actually show the real code postion, an option ```LOG_CALL_STACK``` was added, which logs the whole call stack for events that are logged. When this option is enabled, the log message includes all code-position, up until the position, from where in the code of the user the event was triggered. This can look like this: 
   ```
...
- TSAN_Tracer_no_Source_location: This is the updated TSAN implementation which logs all Thread forks, joins, Memory reads and writes and all Mutex locks and unlocks. In this configuration no source location is written. That means, that it is not shown in the output from where in the programm the logged operation was called.
- TSAN_Tracer_hash_source_location: In this case the location in the program from wehre this operation was called is logged. But this location is already hashed, so fewer characters need to be printed to the console. To test this a hash function was used, that always returns 123.
...
### Execution Time Measurements (All Times in Nanoseconds) and in brackets the factor compared to TSAN_Disabled
...
|------------------------|--------------:|----------------------:|-------------------------------:|---------------------------------:|----------------------------------:|-------------------------:|------------------------:|-----------------------:|-------------------------:|
| **tiny_race**          | 1.010.253.859 | 1.460.940.819 (1,446) |          1.482.788.721 (1,466) |            1.825.988.269 (1,805) |             1.832.168.491 (1,814) |                       71 |                       0 |                     68 |                        4 |
| **mutex_test**         |     5.785.554 |    390.576.568 (67,5) |             401.925.724 (69,4) |              723.422.837 (125,5) |               740.302.229 (127,8) |                       40 |                       6 |                     30 |                        4 |
| **locking_example**    |     5.375.015 |    445.588.896 (82,9) |             464.736.824 (86,4) |              820.420.049 (152,3) |               838.791.299 (156,7) |                       34 |                       4 |                     26 |                        4 |
| **mini_bench_local**   |     6.499.689 |      39.493.128 (6,1) |       13.779.822.959 (2.123,8) |         14.138.933.685 (2.173,3) |          14.239.088.413 (2.194,7) |                   80.417 |                       0 |                 80.409 |                        8 |
| **start_many_threads** |    10.427.980 |      69.409.787 (6,7) |             449.005.205 (43,0) |               824.144.521 (78,9) |                823.809.125 (78,9) |                      504 |                       0 |                    304 |                      200 |
| **mini_bench_shared**  |     3.839.314 |     39.668.881 (10,3) |       14.772.432.203 (3.846,4) |         14.181.331.147 (3.699,7) |          14.501.687.818 (3.773,5) |                   80.684 |                       0 |                 80.676 |                        8 |
| **many_mutexes_bench** |    24.322.063 |      86.276.795 (3,5) |          9.331.037.957 (383,8) |           10.856.998.933 (446,1) |            10.929.595.607 (449,4) |                   40.302 |                  20.000 |                 20.102 |                      200 |
